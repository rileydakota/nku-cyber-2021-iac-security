...
- [Hadolint](https://github.com/hadolint/hadolint) a linting tool for Dockerfiles that also contains some security checks
...
In regards to looking for credentials in code, we are utilizing [detect-secrets](https://github.com/Yelp/detect-secrets), an open source tool developed by Yelp to help identify secrets in code. It has the concept of a baseline file - which is useful if there are potiential false positives/technical debt that cannot be immedietly remediated, but allows you to implement the tool to still block future releases. It comes with a healthy set of supported credential types out of the box, and better yet - is plugin based, meaning you can add your own types of credential detections. The really cool thing about this tool is that some of the plugins actually can [verify](https://github.com/Yelp/detect-secrets/blob/master/docs/plugins.md#Verified-Secrets) if the secret is actually usable - great for demonstrating real risk reduction (eg - this was a real exploitable security issue) and for red team automation. Also - its easy to disable checks that are solely based on "high entropy strings" alone, from my experience this category of scan tended to have the highest false positive ratio. Things like AWS ARNS tend to trigger that, and appear quite often in IaC.

Worth noting that Checkov actually has some secrets detection capabilities itself, but I wanted to show off a different tool, and Checkov only checks Infrastructure-As-Code templates for secrets, detect-secrets can scan pretty much any type of file/codebase, and bundling app code with infra is not an uncommon pattern (plus the aformentioned verification functionality is too cool to pass up).

Other great secrets scanning/detection tools to check out:
